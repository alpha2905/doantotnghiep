...
EPOCHS = 15  
LEARNING_RATE = 2e-5
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "model", "phobert_models")
...
def main():
    ...
    if os.path.exists(sentiment_file):
        ...
        # Split 80/10/10: Train/Val/Test với stratified sampling
        ...
        train_phobert("sentiment_classification", X_train, y_train, X_val, y_val, sentiment_mapping)
        
        # Lưu test set cố định để đánh giá sau này
        test_set_path = os.path.join(DATA_DIR, "sentiment_test_set.jsonl")
        with open(test_set_path, "w", encoding="utf-8") as f:
            for text, label in zip(X_test, y_test):
                f.write(json.dumps({"text": text, "label": int(label)}, ensure_ascii=False) + "\n")
        print(f"💾 Đã lưu Test Set cố định: {test_set_path} ({len(X_test)} mẫu)")
    else:
    ...
    if os.path.exists(aspect_file):
        ...
        # Split 80/10/10: Train/Val/Test với stratified sampling
        X_train_full, X_test, y_train_full, y_test = train_test_split(
            filtered_texts, aspect_encoded, test_size=0.10, random_state=42, stratify=aspect_encoded
        )
        X_train, X_val, y_train, y_val = train_test_split(
            X_train_full, y_train_full, test_size=1/9, random_state=42, stratify=y_train_full
        )
        train_phobert("aspect_classification", X_train, y_train, X_val, y_val, aspect_mapping)
        
        # Lưu test set cố định cho aspect
        aspect_test_path = os.path.join(DATA_DIR, "aspect_test_set.jsonl")
        with open(aspect_test_path, "w", encoding="utf-8") as f:
            for text, label in zip(X_test, y_test):
                f.write(json.dumps({"text": text, "label": int(label)}, ensure_ascii=False) + "\n")
        print(f"💾 Đã lưu Aspect Test Set cố định: {aspect_test_path} ({len(X_test)} mẫu)")
    else:
